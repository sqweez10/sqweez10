...
Observed activity showed multiple UserOperation (`HandleOps`) bundles sweeping claimed TYSM to `chickenattack.base.eth`, which is consistent with automated multi-wallet farming rather than organic community usage.
...
The right framing matters: this was abusive behavior, not a theft narrative.
...
This means the contract direction is understood, the test suite is green, but the production-backed authorization layer still needs to be built and validated.
